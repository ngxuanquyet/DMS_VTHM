...
class DynamicFormField {
  ...
  factory DynamicFormField.fromJson(Map<String, dynamic> json) {
    final typeStr = json['type']?.toString() ?? json['input_type']?.toString() ?? json['data_type']?.toString();
    final type = DynamicFormFieldType.fromString(typeStr);

    List<DynamicFormOption> opts = [];
    if (json['options'] is List) {
      opts = (json['options'] as List).map((o) => DynamicFormOption.fromJson(o)).toList();
    } else if (json['items'] is List) {
      opts = (json['items'] as List).map((o) => DynamicFormOption.fromJson(o)).toList();
    } else if (json['values'] is List) {
      opts = (json['values'] as List).map((o) => DynamicFormOption.fromJson(o)).toList();
    }

    return DynamicFormField(
      code: json['code']?.toString() ?? json['name']?.toString() ?? json['id']?.toString() ?? json['key']?.toString() ?? '',
      label: json['label']?.toString() ?? json['title']?.toString() ?? json['name']?.toString() ?? '',
      type: type,
      placeholder: json['placeholder']?.toString() ?? json['hint']?.toString(),
      helperText: json['helper_text']?.toString() ?? json['description']?.toString(),
      isRequired: json['required'] == true || json['is_required'] == true || json['is_required'] == 1,
      isReadOnly: json['read_only'] == true || json['is_readonly'] == true || json['readonly'] == true,
      ...
      maxPhotos: json['max_photos'] is int ? json['max_photos'] as int : (int.tryParse(json['max_photos']?.toString() ?? '') ?? 5),
      section: json['section']?.toString() ?? json['group']?.toString() ?? json['category']?.toString(),
